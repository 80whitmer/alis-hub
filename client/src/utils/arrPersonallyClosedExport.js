/**
 * Exports the flattened "ARR Personally Closed" table (ArrPersonallyClosedSection,
 * AccountHealthDashboard.jsx) to .xlsx — copy of arrAddedDealsExport.js with
 * a different sheet/file name, same per-file duplication convention this
 * codebase already uses for its other single-purpose exports.
 */
...
export async function exportArrPersonallyClosedDeals(deals) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'alis-hub';
  workbook.created = new Date();

  const sheet = workbook.addWorksheet('ARR Personally Closed');
  sheet.columns = [
  ...
  a.href = url;
  a.download = 'ARR-Personally-Closed.xlsx';
