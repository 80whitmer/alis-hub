 ...
 */
const { hubspotRequest, getPipelineStageLabels, hubspotRecordUrl } = require('./hubspotTickets');
const { getAccountManagerName } = require('./hubspotAccounts');
const { getPinnedNotes, htmlToText, safeHref, decodeEntities } = require('./pinnedNotes');
...
function extractLinks(html, plainText) {
  ...
  const add = (rawUrl, label) => {
    const href = safeHref(decodeEntities(rawUrl));
    if (!href || found.has(href)) return;
    const text = label && label.trim() && label.trim() !== href ? label.trim() : null;
    found.set(href, { url: href, label: text, source: linkSource(new URL(href).hostname) });
  };
...
async function getInternalTickets() {
  ...
  let notes = new Map();
  try {
    notes = await getPinnedNotes(raw.map((t) => t.properties.hs_pinned_engagement_id).filter(Boolean));
  } catch (err) {
  ...
  return raw.map((t) => {
    ...
    const note = p.hs_pinned_engagement_id ? notes.get(String(p.hs_pinned_engagement_id)) : null;
    const description = p.content ? htmlToText(p.content) : null;
    const nextStep = p.next_step || null;
    const noteLinks = (note?.links || []).map((l) => ({
      url: l.href, label: l.label !== l.href ? l.label : null, source: linkSource(new URL(l.href).hostname),
    }));
    return {
      ticketId: t.id,
      ...
      description,
      nextStep,
      pinnedNote: note?.text || null,
      pinnedNoteSegments: note?.segments || null,
